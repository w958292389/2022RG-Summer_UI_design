...
#define LEN_SHOW 13
#define ID_SHOW 0x0100
#define    LEN_HEADER    5       //header length
#define    LEN_CMDID     2        //cmdid length
#define    LEN_TAIL      2				//tail crc16
/*
@brief         sending UI （UART1)
                Ruizhe 2022/5/19
								测试版本
*/

uint32_t sendUI_TimeStamp = 0; //发送时间戳
const uint16_t sendUI_Freq = 10; //发送间隔 （ms)
void JUDGE_Show_Data(void){
	uint8_t Show_Pack[50] = {0}; // as long as larger than 22 which are header:5 + cmdid:2 + data:13 + tail:2
	...
	ShowData.data1 =  100;//????????      //(float)ShootNum;//??????????
	ShowData.data2 = 66.66;//Fric_GetHeatInc();//
	...
	{
		HAL_UART_Transmit(&huart6, &Show_Pack[i], 1, 3 );
	}
